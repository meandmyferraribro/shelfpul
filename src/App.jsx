...
const MP_SPECS = {
  ...
  ebay: { name:"eBay", color:"#E53238", bg:"#FEF2F2", min:500, format:"JPEG, PNG", bgReq:"Any background" },
  etsy: { name:"Etsy", color:"#F1641E", bg:"#FFF4EF", min:800, format:"JPEG, PNG", bgReq:"Any background" },
  mercari: { name:"Mercari", color:"#FF4F5A", bg:"#FFF0F1", min:500, format:"JPEG, PNG", bgReq:"Any background" },
  facebook: { name:"Facebook", color:"#1877F2", bg:"#EFF6FF", min:400, format:"JPEG, PNG", bgReq:"Any background" },
  website: { name:"My Website", color:"#7C3AED", bg:"#F5F3FF", min:800, format:"JPEG, PNG, WEBP", bgReq:"Any background" }
};
...
.mps{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-bottom:24px}.mpo{padding:16px;border:2px solid var(--g200);border-radius:var(--r);cursor:pointer;transition:var(--t);text-align:center}.mpo:hover{border-color:var(--g400)}.mpo.sa{border-color:var(--am);background:#FFF8EC}.mpo.sw{border-color:var(--wm);background:#EBF4FF}.mpn{font-family:var(--fd);font-weight:700;font-size:1rem;margin-top:8px}
...
function AddProductFlow({ onComplete, onCancel }) {
  ...
  const MP_LABELS={amazon:"Amazon",walmart:"Walmart",ebay:"eBay",etsy:"Etsy",mercari:"Mercari",facebook:"Facebook Marketplace",website:"My Website"};const go=()=>{setPub(true);setTimeout(()=>{f.mp.forEach(m=>addProduct({name:f.name,category:f.cat,price:f.price,cost:f.cost,sku:f.sku,description:f.desc,marketplace:MP_LABELS[m]||m,status:"active",images:imgs.map(i=>i.src)}));setPub(false);setDone(true);},2500);};
  ...
  return (
    <div style={{maxWidth:720,margin:"0 auto"}}>
      ...
      {step===1 && <div className="pn"><div className="pnh"><span className="pnt">Product Photos</span><span style={{fontSize:".8rem",color:"var(--g500)"}}>Up to 9</span></div><div className="pnb">
        ...
        {aiDone && <div style={{background:"#ecfdf5",borderRadius:8,marginTop:12,padding:"12px 16px"}}><div style={{fontSize:".85rem",color:"#059669",fontWeight:600,marginBottom:10,textAlign:"center"}}>Product details auto-filled from photo. Click Next to review.</div><div style={{fontSize:".8rem",color:"#065f46",fontWeight:600,marginBottom:8}}>Verify prices on:</div><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{[["Amazon","https://www.amazon.com/s?k=","#FF9900"],["Walmart","https://www.walmart.com/search?q=","#0071DC"],["eBay","https://www.ebay.com/sch/i.html?_nkw=","#E53238"],["Google Shopping","https://www.google.com/search?tbm=shop&q=","#4285F4"],["Etsy","https://www.etsy.com/search?q=","#F1641E"],["Mercari","https://www.mercari.com/search/?keyword=","#FF4F5A"]].map(([name,base,color])=><a key={name} href={base+encodeURIComponent(f.name||"product")} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:4,padding:"5px 10px",borderRadius:6,background:"white",border:"1px solid "+color+"40",color:color,fontSize:".75rem",fontWeight:600,textDecoration:"none"}}>{name}</a>)}</div></div>}{!isPaid && <div style={{textAlign:"center",padding:"16px",background:"#fefce8",borderRadius:8,marginTop:12,border:"1px solid #fde68a"}}><div style={{fontSize:".85rem",fontWeight:600,color:"#92400e",marginBottom:8}}>Upgrade to Pro to auto-detect product details from your photos. Save hours of manual data entry.</div><button className="btn bp bsm">Upgrade to Pro</button></div>}
      ...
      {step===3 && <div className="pn"><div className="pnh"><span className="pnt">Choose Marketplace</span><span style={{fontSize:".8rem",color:"var(--g500)"}}>One or both</span></div><div className="pnb">
        <div className="mps">
          {[["amazon","Amazon","#FF9900","Seller Central"],["walmart","Walmart","#0071DC","Marketplace"],["ebay","eBay","#E53238","Listings"],["etsy","Etsy","#F1641E","Shop"],["mercari","Mercari","#FF4F5A","Listings"],["facebook","Facebook","#1877F2","Marketplace"],["website","My Website","#7C3AED","Own Store"]].map(([key,label,color,sub])=><div key={key} className={"mpo "+(f.mp.includes(key)?"sa":"")} style={f.mp.includes(key)?{borderColor:color,background:color+"10"}:{}} onClick={()=>tgl(key)}><Globe size={32} style={{color:color}}/><div className="mpn" style={{color:f.mp.includes(key)?color:"var(--g700)"}}>{label}</div><div style={{fontSize:".8rem",color:"var(--g500)",marginTop:4}}>{sub}</div>{f.mp.includes(key)&&<CheckCircle size={18} style={{color:color,marginTop:8}}/>}</div>)}
        </div>
  ...
  return (<div className="db">
    ...
    <main className="mn"><div className="tb"><div className="tbl"><button className="mt" onClick={()=>setSb(true)}><Menu size={22}/></button><h1 className="tbt">{tts[tab]||"Dashboard"}</h1></div><div className="tbr"><div className="srch" onClick={()=>setCmdOpen(true)} style={{cursor:"pointer"}}><Search size={18}/><span style={{color:"var(--g400)",fontSize:".9rem",flex:1}}>Search products...</span><span className="cmd-kbd" style={{fontSize:".7rem"}}>⌘K</span></div><div style={{position:"relative"}}><button className="nb" onClick={()=>{setNotifOpen(!notifOpen);setProfOpen(false);}}><Bell size={18}/><span className="nd"/></button><NotificationCenter isOpen={notifOpen} onClose={()=>setNotifOpen(false)} onAction={(type,pid)=>{if(type==="reprice"){setRepriceProduct(pid);setNotifOpen(false);}}} goToTab={(t)=>setTab(t)}/></div><div style={{position:"relative"}}><div className="av" onClick={()=>{setProfOpen(!profOpen);setNotifOpen(false);}}>{user?.avatar||"U"}</div><ProfileDropdown isOpen={profOpen} onClose={()=>setProfOpen(false)} user={user} onLogout={handleLogout} goToTab={setTab}/></div></div></div>
      ...
      <div className="pc" onClick={closeAll}>
        ...
        {tab==="settings"&&<div className="pn" style={{maxWidth:600}}><div className="pnh"><span className="pnt">Account Settings</span></div><div className="pnb"><div className="ig"><label>Full Name</label><input className="inp" defaultValue={user?.name}/></div><div className="ig"><label>Email</label><input className="inp" defaultValue={user?.email}/></div><div style={{borderTop:"1px solid var(--g200)",paddingTop:20,marginTop:20,marginBottom:20}}><div style={{fontSize:".9rem",fontWeight:700,color:"var(--g700)",marginBottom:12}}>Marketplace Connections</div><div className="ig"><label>Amazon Seller ID</label><input className="inp" placeholder="Connect Amazon Seller account"/></div><div className="ig"><label>Walmart Partner ID</label><input className="inp" placeholder="Connect Walmart Partner account"/></div></div><button className="btn bp">Save Changes</button><div style={{borderTop:"1px solid var(--g200)",paddingTop:20,marginTop:20}}><div style={{fontSize:".9rem",fontWeight:700,color:"var(--g700)",marginBottom:16}}>Connected Marketplaces</div><p style={{fontSize:".85rem",color:"var(--g500)",marginBottom:20}}>Enter your API credentials to publish listings directly from ShelfPul.</p>{[["amazon","Amazon","#FF9900","Seller ID","MWS Token"],["walmart","Walmart","#0071DC","Client ID","Client Secret"],["ebay","eBay","#E53238","App ID","Auth Token"],["etsy","Etsy","#F1641E","API Key","Access Token"],["mercari","Mercari","#FF4F5A","Email","Password"],["website","My Website","#7C3AED","Store URL","API Key"]].map(([key,name,color,f1,f2])=><div key={key} style={{border:"1px solid var(--g200)",borderRadius:"var(--r)",padding:16,marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><div style={{width:10,height:10,borderRadius:"50%",background:color}}/><span style={{fontWeight:700,color:color}}>{name}</span></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><div><label style={{fontSize:".75rem",color:"var(--g500)",display:"block",marginBottom:4}}>{f1}</label><input className="inp" style={{fontSize:".85rem"}} placeholder={f1} onChange={e=>localStorage.setItem(key+"_f1",e.target.value)} defaultValue={localStorage.getItem(key+"_f1")||""}/></div><div><label style={{fontSize:".75rem",color:"var(--g500)",display:"block",marginBottom:4}}>{f2}</label><input className="inp" type="password" style={{fontSize:".85rem"}} placeholder={f2} onChange={e=>localStorage.setItem(key+"_f2",e.target.value)} defaultValue={localStorage.getItem(key+"_f2")||""}/></div></div></div>)}</div></div></div>}
      </div>
    </main>
  </div>);
}
